...
func waitForServiceReady(client *http.Client, maxAttempts int, retryDelay time.Duration) (time.Duration, int, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if err == nil && resp.StatusCode == http.StatusOK {
			...
			fmt.Printf("✓ Success after %.2f seconds\n", elapsed.Seconds())
...
func measureFirstRequest(client *http.Client) (time.Duration, bool) {
	...
	req := CreateCartRequest{CustomerID: 99999}
	...
	if err != nil {
		fmt.Printf("✗ Request failed: %v (%.2f ms)\n", err, float64(elapsed.Microseconds())/1000.0)
	...
	if success {
		...
		fmt.Printf("✗ Request failed with status %d: %s (%.2f ms)\n", 
			resp.StatusCode, string(body), float64(elapsed.Microseconds())/1000.0)
	}
...
func verifyStability(client *http.Client, numRequests int) int {
	...
	for i := 1; i <= numRequests; i++ {
		req := CreateCartRequest{CustomerID: 90000 + i}
		body, _ := json.Marshal(req)
...
func TestColdStart(t *testing.T) {
	...
	fmt.Println("  - AWS: terraform apply / azd up")
	fmt.Println("  - LocalStack: docker-compose up")
	...
	if !firstSuccess {
		t.Logf("Warning: First request failed, continuing anyway...")
	...
	if result.ConsecutiveSuccesses < 3 {
		t.Errorf("Service appears unstable: only %d/5 consecutive requests succeeded", result.ConsecutiveSuccesses)
	}
}
...
func TestManualColdStartInstructions(t *testing.T) {
	...
	fmt.Println("   1. Stop LocalStack: docker-compose down")
	fmt.Println("   2. Start timer when you run: docker-compose up")
	fmt.Println("   3. Export BASE_URL: export BASE_URL=http://localhost:8080")
	fmt.Println("   4. Run this test: go test -v -run TestColdStart")
	fmt.Println("   5. Note the total cold start time")
